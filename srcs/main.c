...
static int	open_diner(t_diner *diner)
{
	...
	while (i < diner->shared->nb_philo)
	{
		...
		diner->shared->time_to_die;
		printf("%lli %d %s\n", (get_current_time() - \
		diner->all_the_phils[i]->shared->start), \
		diner->all_the_phils[i]->id, "is thinking");
		if (pthread_create(&(diner->all_the_phils[i]->phil_thread), \
		NULL, phil_routine, diner->all_the_phils[i]))
...
static int	verify_args(t_shared *shared)
{
...
static int	process_args(int argc, char **argv, t_shared *shared)
{
